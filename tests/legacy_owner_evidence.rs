...
const HOME_CONTROLS: [(bool, bool); 3] = [(true, false), (false, false), (false, true)];
...
impl Fixture {
    fn new() -> Self {
        Self::with_home(true, false)
    }

    fn with_home(managed: bool, outside_store: bool) -> Self {
        ...
        let saved = if outside_store {
            physical.join("adopted")
        } else {
            accounts.join("1-legacy")
        };
        private_dir(&saved);
        let fake = physical.join("synthetic-owner-child");
        let marker = physical.join("child-home");
        #[cfg(unix)]
        {
            fs::write(
                &fake,
                b"#!/bin/sh\nprintf '%s' \"$CODEX_HOME\" > \"$SYNTHETIC_OWNER_HOME\"\n",
            )
            .unwrap();
        ...
        write_json(
            ...
            &json!({
                ...
                "accounts": [{"number": 1, "alias": "owner-a", "home": saved,
                    "managed": managed, "shareHistory": false, "enabled": true,
    ...
    fn command(&self, arguments: &[&str]) -> Command {
        ...
        command
            ...
            .args(["--codex-bin"])
            .arg(&self.fake)
            .env("SYNTHETIC_OWNER_HOME", &self.marker)
            .args(arguments);
        command
    }
...
fn same_email_member_cannot_claim_or_export_a_legacy_saved_owner() {
    for (managed, outside) in HOME_CONTROLS {
        let fixture = Fixture::with_home(managed, outside);
        ...
        #[cfg(unix)]
        {
            success(&fixture.run(&["run", "1", "--", "--version"]));
            assert_eq!(
...
fn unusable_transport_hints_isolate_the_owner_without_authorizing_credentials() {
    for (managed, outside) in HOME_CONTROLS {
        for kind in [
            ...
            if kind.starts_with("inferred") {
                unusable.as_object_mut().unwrap().remove("auth_mode");
            }
            match kind {
                "explicit-api" => unusable["auth_mode"] = json!("apikey"),
                "object-api" => unusable["auth_mode"] = json!({"apikey": null}),
                "inferred-api" => unusable["OPENAI_API_KEY"] = json!("synthetic-secret-api-key"),
                "inferred-pat" => unusable["personal_access_token"] = json!("synthetic-secret-pat"),
                "inferred-bedrock" => {
                    unusable["bedrock_api_key"] = json!(["synthetic-secret-bedrock", "region"])
                }
                "malformed-mode" => {
                    unusable["auth_mode"] = json!({"chatgpt": "synthetic-secret-not-unit"})
                }
                _ => unreachable!(),
            }
            ...
            let error = if kind == "malformed-mode" {
                "invalid Codex auth_mode"
            } else {
            ...
            match kind {
                ...
                "malformed" => {
                    hint["tokens"]["id_token"] =
                        json!("dummy.synthetic-secret-not-valid-base64.dummy")
                }
                "truncated" => {
...
#[test]
fn explicit_chatgpt_modes_win_inference_without_changing_historical_owner_resolution() {
    for (managed, outside) in HOME_CONTROLS {
        for mode in [json!("chatgpt"), json!({"chatgpt": null})] {
            let fixture = Fixture::with_home(managed, outside);
            let mut document = login("user-A", EMAIL);
            document["auth_mode"] = mode;
            document["OPENAI_API_KEY"] = json!("synthetic-secret-cached-api-key");
            write_json(&fixture.saved.join("auth.json"), &document);
            let before = fixture.snapshot();
            assert!(fixture.json(&["status", "--json"])["active"].is_null());
            let listed = fixture.json(&["list", "--json"]);
            assert_eq!(listed["accounts"][0]["userId"], "user-A");
            assert_eq!(listed["accounts"][0]["loginStatus"], "present");
            let (output, file) = fixture.export("explicit-chatgpt.json");
            success(&output);
            let backup: Value = serde_json::from_slice(&fs::read(file).unwrap()).unwrap();
            assert_eq!(backup["accounts"][0]["auth"], document);
            assert!(!String::from_utf8_lossy(&output.stdout).contains("synthetic-secret"));
            assert!(!String::from_utf8_lossy(&output.stderr).contains("synthetic-secret"));
            assert_eq!(fixture.snapshot(), before);
        }
