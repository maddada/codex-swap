...
xswap follows Codex's authentication-mode precedence: explicit `chatgpt` permits stored credentials for other modes, but a missing/null mode with a non-null API key, personal access token, or Bedrock credential is rejected. Legacy ChatGPT files with missing/null mode and missing/null material for those other modes remain supported. Existing incompatible snapshots show `invalid_credentials`; use `xswap login <slot-or-alias>` to sign in with the registered ChatGPT identity. Their credentials and account metadata stay in place until a verified login succeeds.
